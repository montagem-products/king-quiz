...
async function generateQuiz(genre, kingName) {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 1000,
      system: `あなたはクイズ出題者です。ジャンルに沿ったパーソナルで面白いクイズを1問だけ作ってください。JSONのみで返答（マークダウン不要）。形式: {"question":"問題文","hint":"ヒント（1行、任意）"} 必ず問題文の主語を「${kingName}」にしてください。問題パターンは多様にしてください（「好きなもの」に偏らないこと）。例：「${kingName}が絶対に許せないことは？」「${kingName}が最近ハマっていることは？」「${kingName}が一番恥ずかしかった瞬間は？」「${kingName}が密かにやってみたいことは？」「${kingName}が嫌いな食べ物は？」「${kingName}が朝一番にすることは？」「${kingName}が友達に言えない秘密は？」「${kingName}が学生時代に一番サボったことは？」「${kingName}が絶対に食べたくないものは？」「${kingName}が宝くじ1億当たったら？」「${kingName}が無人島に持っていくものは？」「${kingName}が人生で後悔していることは？」。好き・嫌い・エピソード・もしも・秘密・習慣・大喜利など幅広いパターンをランダムに使ってください。大喜利系の例：「${kingName}といえば？（一言で）」「${kingName}を動物に例えると？」「${kingName}が書いた本のタイトルは？」「${kingName}の隠れたあだ名は？」「${kingName}が主人公のドラマのタイトルは？」「${kingName}が絶対言わなそうな一言は？」。回答者が正解を知らなくても想像と笑いで盛り上がれる問題にしてください。`,
  ...
  return (
    <div style={S.page}>
      <div style={{ marginBottom: 40, paddingTop: 20 }}>
        ...
        <h1 style={S.h1}>○○理解され王</h1>
        <p style={{ color: C.muted, fontSize: 13, marginTop: 6, marginBottom: 28 }}>理解され王に認められた家臣が勝者だ！</p>
      </div>
...
function WaitingScreen({ roomCode, playerId, onGameStart }) {
  ...
  const startGame = async () => {
    ...
    await update(ref(db, `rooms/${roomCode}`), {
      phase: "loading", king: king.id, kingName, round: 1,
      genre, answers: {}, answeredCount: 0,
    });
    let quiz;
    try { quiz = await generateQuiz(genre, kingName); }
    catch { quiz = { question: `${kingName}の好きな${genre}は？`, hint: "" }; }
    await update(ref(db, `rooms/${roomCode}`), { quiz, phase: "roleReveal" });
    setStarting(false);
  };
    ...
    <div style={S.page}>
      ...
      <div style={{ ...S.card, background: isKing ? C.accentDim : C.greenDim, border: `2px solid ${isKing ? C.accent : C.green}`, textAlign: "center", padding: "32px 20px", marginBottom: 24 }}>
        ...
        <p style={{ color: C.muted, fontSize: 13, margin: 0 }}>
          {isKing ? "家臣たちの回答を見てポイントを付けよう！" : "正直に答えて理解され王に認められよう！"}
        </p>
      </div>
      {isHost
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ marginTop: 16 }}>
        {isHost
          ? <button style={S.btn()} onClick={goToScoring}>👑 {room.kingName}の採点へ →</button>
          : <p style={{ color: C.muted, textAlign: "center", fontSize: 13 }}>ホストが採点に進めます</p>}
      </div>
    </div>
  );
}
...
function ScoredScreen({ room, playerId }) {
  ...
  return (
    <div style={S.page}>
      <p style={S.tag}>採点結果 · round {room.round}</p>
      <div style={S.quizCard}><p style={S.quizQ}>{room.quiz?.question}</p></div>
      {retainers.map(p => {
...
function ResultScreen({ room, playerId }) {
  ...
  const nextRound = async () => {
    ...
    catch { quiz = { question: `${room.kingName}の好きな${genre}は？`, hint: "" }; }
    await update(ref(db, `rooms/${room.code}`), { quiz, phase: "quiz" });
  };
